...
import { transportFormStyle } from "../../styles/inventoryStyles/transportFormStyle";
...
const TransportForm = () => {
  ...
  useEffect(() => {
    if (editingItem) {
      setFormData({
        transportType: editingItem.transportType || "",
        capacity: editingItem.capacity || "",
        costPerTrip: editingItem.costPerTrip !== undefined ? String(editingItem.costPerTrip) : "",
      });
    }
  }, [editingItem]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem?.id) {
        await updateDoc(doc(db, 'transport', editingItem.id), {
  ...
  return (
    <SafeAreaView style={transportFormStyle.container} onLayout={onLayoutRootView}>
      <ScrollView
      ...
      >
        <Text style={transportFormStyle.moduleTitle}>Transporte</Text>

        <Text style={transportFormStyle.label}>Tipo de transporte</Text>
        <Animated.View
          style={[
            transportFormStyle.inputContainer,
            errors.transportType && transportFormStyle.errorInput,
            { transform: [{ translateX: shakeAnim.transportType }] },
          ]}
        >
          <TextInput
            style={transportFormStyle.inputText}
        ...
        {errors.transportType && (
          <Text style={transportFormStyle.errorText}>
            {errors.transportType}
          </Text>
        )}
        ...
        <FormButton
          title={loading ? (editingItem ? "Actualizando..." : "Guardando...") : (editingItem ? "Actualizar transporte" : "Guardar transporte")}
          onPress={handleSave}
          loading={loading}
        />
      </ScrollView>
